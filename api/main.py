...
def handle_supabase_error(remote_addr, error_message):
    print(f"[{remote_addr}] {error_message}")
...
def get_matches():
    ...
    try:
        ...
        return handle_supabase_error(request.remote_addr, f"Error getting matches {e}")
...
def get_points_for_user(id):
    ...
    try:
        ...
        return handle_supabase_error(request.remote_addr, f"Error getting points for user {id}: {e}")
...
def get_scoreboard():
    ...
    try:
        ...
        return handle_supabase_error(request.remote_addr, f" Error fetching users: {e}")
    return users
...
def login():
    ...
    try:
        ...
        return handle_supabase_error(request.remote_addr, f"Error login user {login}: {e}")
...
def post_knockout_matches():
    ...
    for num, match in enumerate(matches):
        ...
        try:
            ...
            return handle_supabase_error(request.remote_addr, f"Error getting id for team {team_one}: {e}")
        try:
            ...
            return handle_supabase_error(request.remote_addr, f"Error getting id for team {team_two}: {e}")
        ...
        try:
            ...
            return handle_supabase_error(request.remote_addr, f"Error inserting knockout match: {e}")
...
def post_group_matches():
    ...
    for num, match in enumerate(matches):
        ...
        try:
            ...
            return handle_supabase_error(request.remote_addr, f"Error inserting group matches {match}: {e}")
    ...
    for group in data:
        ...
        for team in teams:
            ...
            try:
                ...
                return handle_supabase_error(request.remote_addr, f"Error adding team: {team}: {e}")
...
def post_predictions():
    ...
    try:
        ...
        return handle_supabase_error(request.remote_addr, f"Error getting user predictions for user id: {user_id}, match id: {match_id}: {e}")
    ...
    if already_predicted.data is not None:
        ...
        try:
            ...
            return handle_supabase_error(request.remote_addr, f"Error updating user predictions for user id: {user_id}, match id: {match_id}: {e}")
...
def get_predictions(user_id):
    ...
    try:
        ...
        return handle_supabase_error(request.remote_addr, f"Error getting user predictions for user id: {user_id}: {e}")
...
def get_match_predictions(match_id):
    ...
    try:
        ...
        return handle_supabase_error(request.remote_addr, f"Error getting all predictions for match id {match_id}: {e}")
...
def update_result():
    ...
    match_id, real_team_one_goals, real_team_two_goals = match_result_info["match_id"], match_result_info["team_one_goals"], match_result_info["team_two_goals"]

    try:
        table_before = supabase.table("users").select("id", "login", "points", "correct_exact_result").execute()
    except Exception as e:
        return handle_supabase_error(request.remote_addr, f"Error getting users: {e}")
    ...
    match_result = 1 if real_team_one_goals > real_team_two_goals else 2 if real_team_one_goals < real_team_two_goals else 0
    try:
        supabase.table("matches").update({"team_one_goals": real_team_one_goals, "team_two_goals": real_team_two_goals, "result": match_result}).eq("id", match_id).execute()
        print(f"[{request.remote_addr}] Updated result for {match_id}, team_one_goals: {real_team_one_goals} team_two_goals: {real_team_two_goals}, result: {match_result}")
    except Exception as e:
        return handle_supabase_error(request.remote_addr, f"Error updating results for match id {match_id}: {e}")
    
    try:
        users_predictions = supabase.table("predictions").select(
                "user_id",
                "result",
                "team_one_goals",
                "team_two_goals",
        ...
        return handle_supabase_error(request.remote_addr, f"Error getting predictions for match id {match_id}: {e}")
    ...
    for user_prediction in users_predictions.data:
        ...
        if match_result == user_result:
            points_to_add += 1
        if user_team_one_goals == real_team_one_goals and user_team_two_goals == real_team_two_goals:
            points_to_add += 3
            exact_result_to_add += 1

        current_user_points = supabase.table("users").select("login", "points", "correct_exact_result").eq("id", user_id).execute()
        ...
        try:
            ...
            return handle_supabase_error(request.remote_addr, f"Error updating user points for user id {user_id}, username: {username}: {e}")
